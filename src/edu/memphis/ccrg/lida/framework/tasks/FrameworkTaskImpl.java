...
public abstract class FrameworkTaskImpl extends LearnableImpl implements FrameworkTask {
    ...
    private static final Logger logger = Logger.getLogger(FrameworkTaskImpl.class
            .getCanonicalName());
    ...
    /**
     * Associated modules with this {@code FrameworkTask}. The modules that will
     * be associated are based on the "associatedModules" parameter value for
     * this {@code FrameworkTask}, which is set in the factory configuration
     * file.
     * 
     * @param moduleMap
    ...
    public void initAssociatedModules(Map<ModuleName, FrameworkModule> moduleMap) {
        ...
        String assocModulesParam = getParam("associatedModules", "");
        if (!(assocModulesParam == null || assocModulesParam.isEmpty())) {
            String[] assocModules = assocModulesParam.split(",");
            for (String m : assocModules) {
                ModuleName name = ModuleName.getModuleName(m);
                FrameworkModule module = moduleMap.get(name);
