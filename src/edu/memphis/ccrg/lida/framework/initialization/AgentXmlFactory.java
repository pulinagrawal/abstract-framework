...
public class AgentXmlFactory implements AgentFactory {
    ...
    static void getTaskSpawner(Element element, TaskManager tm,
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Exception \"" + e.toString()
                    + "\" occurred during creation of object of class " + className, 0L);
        }
        if (ts == null) {
            return;
        }
    ...
    List<FrameworkModule> getModules(Element element, List<Object[]> toAssoc,
            List<Object[]> toInit, Map<String, TaskSpawner> spawners, List<TaskData> toRun) {
        ...
        if (nl != null && nl.size() > 0) {
            ...
            if (list != null && list.size() > 0) {
                for (Element moduleElement : list) {
                    FrameworkModule module = getModule(moduleElement, toAssoc, toInit,
                            spawners, toRun);
                    if (module != null) {
                        modules.add(module);
                    }
    ...
    FrameworkModule getModule(Element moduleElement, List<Object[]> toAssoc,
        ...
        try {
            ...
            if (e instanceof ClassNotFoundException) {
                ...
                logger.log(Level.SEVERE,
                        "Exception occurred during creation of object of class " + className
                                + " Cause: " + e, 0L);
                e.printStackTrace();
            }
            return null;
        ...
        try {
            ...
            logger.log(Level.WARNING, "Module: " + name + " threw exception " + e
                    + " during call to init()", TaskManager.getCurrentTick());
            e.printStackTrace();
        }
    ...
    static void getListener(Element moduleElement, FrameworkModule topModule) {
        ...
        String listenerType = XmlUtils.getTextValue(moduleElement, "listenertype");
        ...
        if (sourceModuleName == null) {
            logger.log(Level.WARNING, "Source module name: " + sourceModule + " is not valid.",
                    0L);
            return;
        }
        ...
        if (listenerClass.isInstance(listener)) {
            ...
            logger.log(Level.INFO, "Added Listener of type: " + listenerType + " for " + module
                    + " -> " + listener, 0L);
        } else {
            logger.log(Level.WARNING, "Listener: " + listenername + " is not a valid "
                    + listenerType + " listener.", 0L);
            return;
        }
    }
    ...
    static void associateModules(List<Object[]> toAssoc, FrameworkModule topModule) {
        ...
        for (Object[] vals : toAssoc) {
            ...
            if (moduleName == null) {
                logger.log(Level.WARNING, "Module associated module name: " + assocModule
                        + " is not valid.", 0L);
                break;
            }
            ...
            if (module != null) {
                ...
                logger.log(Level.SEVERE, "Could not obtain " + module
                        + ".  Module will NOT be associated to " + initializable, 0L);
            }
        ...
        for (Object[] vals : toInit) {
            ...
            if (initializer != null) {
                try {
                    ...
                    logger.log(Level.SEVERE, "Exception occurred running initializer: "
                            + initializerClassName, TaskManager.getCurrentTick());
                    e.printStackTrace();
                }
            }
        }
